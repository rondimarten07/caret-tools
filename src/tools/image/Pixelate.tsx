...
export default function Pixelate() {
  ...
  useEffect(() => { render(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [block]);
  ...
  function render() {
    ...
    const small = Math.max(1, Math.floor(W / block));
    canvas.width = W;
    canvas.height = H;
    ...
    tmp.width = small;
    tmp.height = Math.max(1, Math.floor((H / W) * small));
  ...
  function download() {
    ...
    canvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `pixelated-${block}.png`;
      a.click();
  ...
  return (
    <ToolShell title="Image Pixelate" description="Mosaic any image with adjustable block size." category={categoryMap.image}>
        ...
        <>
          <Card className="space-y-3 p-3">
            <div className="flex items-center gap-3">
              <Label className="text-sm w-32">Pixel blocks: {block}</Label>
              <input type="range" min={4} max={150} step={1} value={block} onChange={(e) => setBlock(Number(e.target.value))} className="flex-1" />
